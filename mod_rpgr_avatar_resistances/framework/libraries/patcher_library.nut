::AR.Patcher <-
{
	...
	{
		if (::AR.Manager.isModernHooksInstalled())
		{
			::AR.Integrations.ModernHooks.hook(_path, _function);
			return;
		}

		::AR.Integrations.ModdingScriptHooks.hook(this.formatPath(_path), _function);
	}
	...
	function hookBase( _path, _function )
	{
		if (::AR.Manager.isModernHooksInstalled())
		{
			::AR.Integrations.ModernHooks.hookBase(_path, _function);
			return;
		}

		::AR.Integrations.ModdingScriptHooks.hookBase(this.formatPath(_path), _function);
	}
	...
	function hookTree( _path, _function )
	{
		if (::AR.Manager.isModernHooksInstalled())
		{
			::AR.Integrations.ModernHooks.hookTree(_path, _function);
			return;
		}

		::AR.Integrations.ModdingScriptHooks.hookTree(this.formatPath(_path), _function);
	}
	...
	function wrap( _object, _methodName, _function, _procedure = "overrideReturn" )
	{
		...
		_object.rawset(_methodName, function( ... )
		{
			...
			{
				::AR.Standard.log(format("An invalid number of parameters were passed to %s, aborting wrap procedure.", _methodName), true);
				return;
			}

			local argumentsArray = ::AR.Patcher.prependContextObject(this, vargv);
			return ::AR.Patcher[_procedure](this, _function, originalMethod, argumentsArray);
		});
	}
};